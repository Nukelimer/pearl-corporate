...
export function PracticeCard({
  ...
  return (
    <div
      ...
      <h3 className="text-lg font-semibold text-primary uppercase mb-2">{title}</h3>
      <p className="text-sm text-center text-muted-foreground leading-relaxed">
        {description}
      </p>
      {learnMoreUrl && <Link href={learnMoreUrl} className="text-accent flex items-center gap-2 text-xs pt-4">Learn More  <ArrowRight size={14}/> </Link>}
    </div>
  );
}
