...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_article_authors_role" AS ENUM('Managing Partner', 'Associate', 'Trainee', 'NYSC Intern', 'Guest Writer');
  ...
  CREATE TABLE "articles" (
  	...
  	"read_time_in_minutes" varchar DEFAULT 0,
  ...
  CREATE TABLE "article_authors" (
  	...
  	"role" "enum_article_authors_role" DEFAULT 'NYSC Intern' NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
