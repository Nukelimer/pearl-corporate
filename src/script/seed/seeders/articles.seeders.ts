...
import { STATUS_OPTIONS } from "@/src/collections/Articles/constants";
import { MAX_SUMMARY_LENGTH } from "@/src/collections/hooks/constants";
...
export async function seedArticles(payload: Payload) {
  ...
  for (let i = 0; i < ARTICLES_COUNT; i++) {
    try {
      ...
      if (!image) {
        console.warn(
          "Stopped seeding article author because no image was created"
        );
        return;
      }
